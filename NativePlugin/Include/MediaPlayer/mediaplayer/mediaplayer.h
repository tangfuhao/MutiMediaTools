#ifndef __MEDIAPLAYER_FFPLAYER_H__
#define __MEDIAPLAYER_FFPLAYER_H__
...
#define MSG_IMAGE_RENDER   (('I' << 24) | ('M' << 16) | ('G' << 8) | ('S' << 0))
...
enum {
    ...
    PARAM_CLEAR_RENDER_STEPFORWARD,
...
typedef struct {
    ...
    int audio_read_channels; //读取时的采样率
    int audio_read_sampleRate; //读取时的采样率
