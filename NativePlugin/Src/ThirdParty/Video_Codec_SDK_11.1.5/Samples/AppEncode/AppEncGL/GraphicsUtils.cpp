...
bool SetupGLXResourcesWithShareContext(GLXContext shareContext,Display* display,GLXFBConfig config,int pBufferWidth,int pBufferHeight)
{
    //获取 扩展 api  创建无窗口的context
    typedef GLXContext (*glXCreateContextAttribsARBProc)(Display*, GLXFBConfig, GLXContext, Bool, const int*);
    glXCreateContextAttribsARBProc glXCreateContextAttribs = NULL;
    glXCreateContextAttribs = (glXCreateContextAttribsARBProc) glXGetProcAddressARB((const GLubyte *) "glXCreateContextAttribsARB");
    ...
    GLXContext glContext = glXCreateContextAttribs(display, config, shareContext, 1, context_attribs);
    ...
    pbuffer = glXCreatePbuffer(display, config, pbufferAttribs);
    //XFree(config);
    XSync(display, False);

    //绑定到pbuffer
    glXMakeContextCurrent(display, pbuffer, pbuffer, glContext);
