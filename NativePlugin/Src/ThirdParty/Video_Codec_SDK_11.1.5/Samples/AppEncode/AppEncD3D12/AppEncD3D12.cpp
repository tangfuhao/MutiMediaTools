...
*  for H264 encode and HEVC encode.
*/
...
public:
    ...
    std::streamsize ReadInputFrame(std::ifstream& fpBgra, const NvEncInputFrame* encoderInputFrame, NV_ENC_FENCE_POINT_D3D12* pInpFencePoint)
    {
...
void Encode(ID3D12Device* pDevice, int nWidth, int nHeight, NvEncoderInitParam encodeCLIOptions, std::ifstream& fpBgra, std::ofstream& fpOut)
{
    ...
    while (true)
    {
        ...
        std::streamsize nRead = pUploadInput->ReadInputFrame(fpBgra, encoderInputFrame, pInpFencePOint);
