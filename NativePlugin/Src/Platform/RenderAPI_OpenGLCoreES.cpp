...
#		endif
		...
		this->openglConfig = malloc(sizeof(OpenGLConfig));
		OpenGLConfig *thisOpenglConfig = (OpenGLConfig*)this->openglConfig;
		GLXContext ctx = glXGetCurrentContext();
		Display* dpy = glXGetCurrentDisplay();
		if(ctx == NULL){
			LOG(INFO) << "ctx is not exist!";
		}else{
			LOG(INFO) << "ctx is exist!";
		}

		if(dpy == NULL){
			LOG(INFO) << "dpy is not exist!";
		}else{
			LOG(INFO) << "dpy is  exist!";
		}

		int major;
		int minor;
		if(glXQueryVersion(dpy,&major,&minor)){
			LOG(INFO) << "glXQueryVersion: major:" << major << " minor:"<<minor;
		}
		LOG(INFO) << "glXQueryVersion: major:" << major << " minor:"<<minor;

		int attr[3] = { GLX_FBCONFIG_ID, 0, None };
		if (glXQueryContext(dpy, ctx, GLX_FBCONFIG_ID, &attr[1]) != Success) {
			fprintf(stderr, "Failed to query GLX_FBCONFIG_ID\n");
		}
		int cnt = 0;
		GLXFBConfig *config = glXChooseFBConfig (dpy, DefaultScreen (dpy), attr, &cnt);
		LOG(INFO) << "glXChooseFBConfig: cnt:" << cnt ;

		XVisualInfo *vi = glXGetVisualFromFBConfig (dpy, config[0]);
		LOG(INFO) << "Chosen visual ID = " << vi->visualid;

		thisOpenglConfig->dpy = dpy;
		thisOpenglConfig->fbconfig = config[0];
		thisOpenglConfig->majorGLXContext = ctx;
		XFree(config);
		XFree(vi);
